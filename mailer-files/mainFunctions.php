<?php
    ...
    function iInsertMainMattress($group_id,$type, $pf_name, $pf_url, $title,$Product_brand,$content,$price,$Quantity,$tag_id,$i_order,$size,$material,$comfort_level,$mattress_type,$recommended) {
        //! Message Loggin
        comment_message_log('Start of Function : '. __FUNCTION__);
        ...
        //! Query
        echo $sQuery = "INSERT INTO `main` (`id`,`group_id`,`type`, `pf_name`, `pf_url`, `title`,`Product_brand`,`content`,`price`,`Quantity`,`tag_id`,`i_order`,`size`,`material`,`comfort_level`,`mattress_type`,`recommended`) VALUES (NULL,'$group_id','$type', '$pf_name', '$pf_url', '$title','$Product_brand','$content','$price','$Quantity','$tag_id','$i_order','$size','$material','$comfort_level','$mattress_type','$recommended');";
     ...
     function iUpdateMainMattress($group_id,$type,$title,$Product_brand,$content,$price,$Quantity,$tag_id,$i_order,$size,$material,$comfort_level,$mattress_type,$recommended,$made_up_for,$warranty,$colors)
    {
       comment_message_log('Start of Function : '. __FUNCTION__);
       ...
       //! Query
       $sQuery = "UPDATE `main` SET `title` = '$title', `Product_Brand` = '$Product_brand', `content` = '$content', `price` = '$price', `Quantity` = '$Quantity', `tag_id` = '$tag_id', `size` = '$size', `material` = '$material', `comfort_level` = '$comfort_level',`mattress_type`='$mattress_type',`i_order` = '$i_order',`recommended`='$recommended',`made_up_for`='$made_up_for',`warranty`='$warranty',`colors`='$colors'";
  ...
  function aGetAllMainByType($type)
    ...
    function aGetAllDetailsByGroup($type,$id)
    {
      ...
      function aGetSimilarProducts($type,$group_id,$material,$comfort_level,$pillow_type)
    {
        //! Message Loggin
        comment_message_log('Start of Function : '. __FUNCTION__);
        ...
        */
        ...
        //! Query
        $sQuery = "SELECT * FROM `main` WHERE (`comfort_level`='$comfort_level' or `pillow_type`='$pillow_type' or `material`='$material') and (`group_id`, `id`) IN ( SELECT  `group_id`, MIN(`id`) FROM  `main` GROUP BY `group_id`) AND `type` = '$type' and `group_id` not in ('$group_id');";
